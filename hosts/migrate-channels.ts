...
function logPlan(p: Plan): void {
    if (p.from === p.to && p.to !== "archive")
        return;
    if (p.from === p.to && p.to === "archive") {
        console.log(`${p.from} = archive     ${p.reason.padEnd(48)} ${p.title.slice(0, 60)} origin=${p.origin ?? "?"}`);
        return;
    }
    const extra = p.to === "archive"
        ? p.origin === "external"
            ? ` origin=external outlet=${p.outlet ?? "—"} url=${p.externalUrl ?? "MISSING"}`
            : ` origin=${p.origin ?? "first-party"}`
        : "";
    console.log(`${p.from} → ${p.to.padEnd(12)} ${p.reason.padEnd(48)} ${p.title.slice(0, 60)}${extra}`);
}
function patchPlan(tx: ReturnType<ReturnType<typeof createClient>["transaction"]>, p: Plan): boolean {
    const needsChannel = p.from !== p.to;
    const needsArchiveMeta = p.to === "archive";
    if (!needsChannel && !needsArchiveMeta)
        return false;
    if (p.to === "archive") {
        ...
        if (p.origin === "external") {
            if (p.externalUrl)
                set.externalUrl = p.externalUrl;
            if (p.outlet)
                set.outlet = p.outlet;
            tx.patch(p.id, { set });
        }
        else {
            tx.patch(p.id, { set, unset: ["externalUrl", "outlet", "commentary", "archiveSnapshot"] });
        }
    }
    else {
        tx.patch(p.id, {
...
async function main() {
    ...
    const filter = all ? `*[_type == "post"]` : `*[_type == "post" && channel == "press"]`;
    ...
    for (const p of plans)
        byChannel.set(p.to, (byChannel.get(p.to) ?? 0) + 1);
    console.log(`classified ${plans.length} posts${all ? " (--all)" : " (channel=press only)"}`);
    console.log("by section:", [...byChannel.entries()]
        .sort((a, b) => b[1] - a[1])
        .map(([k, v]) => `${k}=${v}`)
        .join("  "));
    console.log("");
