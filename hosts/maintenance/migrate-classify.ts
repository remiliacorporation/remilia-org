...
const LOW_SCORE_HOST =
  /businesswire|globenewswire|issuewire|politico|youtube|reddit|wikipedia/;
const OUTLET_RES = [
...
const tags = (p: PostRow) =>
  new Set((p.tags ?? []).filter((t): t is string => !!t));
...
export function pickExternalUrl(
  ...
  const urls = (hrefs ?? []).filter(
    (h): h is string => !!h && h.startsWith("http") && !remilia(h),
  );
  ...
  const scored = urls.map((u) => {
    ...
    let score = 0;
    try {
      const depth = new URL(u).pathname
        .replace(/\/$/, "")
        .split("/")
    ...
    if (SOCIAL.has(h)) score -= 5;
    if (LOW_SCORE_HOST.test(h)) score -= 2;
    return { u, score };
  });
...
const pressRelease = (title: string, t: Set<string>) =>
  t.has("Press Release") || /^Press Release:/i.test(title);
const orgCorporate = (title: string) =>
  /\bcondemns?\b|\bstatement\b|\bunrepresentative\b/i.test(title);
const externalCoverage = (title: string, t: Set<string>) =>
  t.has("Interview") ||
  /^(Feature|News|Interview|Cultural Coverage|Event Coverage|Thought Leadership):/i.test(
    title,
  ) ||
  (t.has("Press") && t.has("Feature") && !t.has("Press Release"));
const devBlog = (title: string, slug: string) =>
  /^RemiliaNET\b/i.test(title) ||
...
const devUpdates = (title: string, slug: string) =>
  /^RemiliaNET\b/i.test(title) ||
  /RemiliaNET Alpha/i.test(title) ||
...
const comNews = (title: string, slug: string) =>
  /exegesis/i.test(title) ||
  /exegesis/i.test(slug) ||
  (/pre-release announcement/i.test(title) &&
    /miya|exegesis|book|manuscript/i.test(title));
...
const rules: ((p: PostRow, t: Set<string>) => Hit | null)[] = [
  ...
  (p, t) =>
    devBlog(p.title, p.slug)
      ? {
          to: "dev-blog",
          reason: "RemiliaNET Alpha / wiki / miladychan → dev-blog",
        }
      : null,
  (p, t) =>
    devUpdates(p.title, p.slug)
      ? { to: "dev-updates", reason: "net software product note" }
      : null,
  (p, t) =>
    pressRelease(p.title, t) && !orgCorporate(p.title)
      ? { to: "news", reason: "brand / fashion / lifestyle / publishing" }
      : null,
  (p, t) =>
    comNews(p.title, p.slug)
      ? { to: "news", reason: "brand / fashion / lifestyle / publishing" }
      : null,
  (p, t) =>
    pressRelease(p.title, t)
      ? { to: "press", reason: "org corporate press statement" }
      : null,
  (p, t) =>
    externalCoverage(p.title, t) || (t.has("Press") && !t.has("Press Release"))
      ? archiveExternal(p, "press coverage / interview → archive")
      : null,
  (p, t) =>
    UPDATES_SLUGS.has(p.slug) ||
    (/corporate memo/i.test(p.title) && /christmas|missive/i.test(p.title))
      ? { to: "updates", reason: "company essay / memo" }
      : null,
  (p, t) =>
    THOUGHT_PARAGRAPH.has(p.slug) || THOUGHT_GHOST.has(p.slug)
      ? { to: "thought", reason: "org/product thought leadership" }
      : null,
  (p, t) =>
    t.has("Thought") || t.has("Note") || t.has("Review") || t.has("NFT")
      ? { to: "thought", reason: "theory/NFT tag" }
      : null,
  (p, t) =>
    t.has("Project") && !devUpdates(p.title, p.slug)
      ? { to: "thought", reason: "project design essay" }
      : null,
  (p, t) =>
    p.source === "substack" || p.source === "paragraph"
      ? {
          to: "archive",
          reason: `secondary ${p.source} → archive`,
          origin: "first-party",
        }
      : null,
  (p, t) =>
    t.has("Guide")
      ? {
          to: "archive",
          reason: "secondary guide → archive",
          origin: "first-party",
        }
      : null,
  (p, t) =>
    t.has("Announcements")
      ? { to: "updates", reason: "tag:Announcements" }
      : null,
  (p, t) =>
    p.source === "ghost"
      ? { to: "thought", reason: "untagged ghost → thought" }
      : null,
];
