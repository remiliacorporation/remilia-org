...
const BODY_TEXT =
  "How vaults work on RemiliaNET, in enough detail that an agent without JavaScript still reads real content. ".repeat(
    6,
  );
...
test("auditIndexability enforces both directions", () => {
  ...
  const noindexed = GOOD_PAGE.replace(
    "</head>",
...
test("generated feeds pass their own audits (generator ↔ auditor lockstep)", () => {
  const rssXml = rss(
    { channel: "dev-blog", title: "Devblog", description: "d" },
    POSTS,
  );
  ...
  const sitemapXml = sitemap([
    { loc: "https://www.remilia.net/blog/vaults", lastmod: "2026-08-01" },
  ]);
  ...
  const llms = llmsTxt({
    ...
    whenToUse: [
      "Cite RemiliaNET engineering decisions, changelogs, and vault mechanics.",
    ],
    posts: POSTS,
    citeElsewhere: [
      {
...
test("feed discovery and article semantics catch omissions", () => {
  ...
  assert.ok(
    auditArticleSemantics("<main><h1>V</h1></main>").some((e) =>
      e.includes("<article>"),
    ),
  );
  assert.ok(
    auditArticleSemantics("<article>x</article>").some((e) =>
      e.includes("<time"),
    ),
  );
});
...
test("audit404 rejects soft-404s and bare 404 bodies", () => {
  ...
  assert.deepEqual(
    audit404(404, "Not found. See /blog/sitemap.xml or /llms.txt"),
    [],
  );
...
test("auditRobots requires the channel sitemap line", () => {
  assert.deepEqual(
    auditRobots(
      "User-agent: *\nSitemap: https://www.remilia.net/blog/sitemap.xml",
      "dev-blog",
    ),
    [],
  );
...
test("page audits accept typed JSON-LD graphs", () => {
  const html = GOOD_PAGE.replace('{"@context":"https://schema.org","@type":"BlogPosting","headline":"Vaults"}', '{"@context":"https://schema.org","@graph":[{"@type":"BlogPosting","headline":"Vaults"}]}');
  assert.deepEqual(auditPage(html, "https://www.remilia.net/blog/vaults"), []);
});
