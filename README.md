...
Corporate pages are hand-authored in `deploy/src/`. Edit `.layer-base` and run `pnpm bake` to regenerate `deploy/`; the ink spread is printed by the `print-fx` filter, not by a second layer. Keep implementation explanations here rather than in code comments. Generated literature under `deploy/{updates,press,thought,archive}/` is ignored by Git.
