...
| .net | devupdates, devblog | `/updates`, `/blog` |

Posts use schema field `channel` = section id (`devupdates` so GROQ never
...
- **Dev updates** (.net, id `devupdates`) — product/network routine notes.
